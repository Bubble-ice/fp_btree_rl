...
class FplanEnv {
   ...
   protected:
    ...
    double min_cost(vector<double>& costs);
    double avg_cost(vector<double>& costs);
    double norm_cost(double cost);
    void calc_d_mods_info(Modules_Info& mods_a,
                          Modules_Info& mods_b,
                          double& changed_rate,
                          double& changed_area);
   ...
   private:
    ...
    void show_statu(py::array_t<double>& statu);
};
...
SAResult run_with_sa(B_Tree_Ext fp,
                     int times = 400,
                     int local = 7,
                     float init_temp = 0.9f,
                     float term_temp = 0.1f,
                     float alpha = 1.0f,
                     const char* outfile = "",
                     bool is_debug = false);

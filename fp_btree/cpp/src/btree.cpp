...
bool B_Tree::legal_tree(int p, int n, int& num) {
    ...
    if (nodes[n].parent != p)
        return false;
    if (nodes[n].left != NIL)
        if (legal_tree(n, nodes[n].left, num) != true)
            return false;

    if (nodes[n].right != NIL)
        if (legal_tree(n, nodes[n].right, num) != true)
            return false;
...
void B_Tree::testing() {
    ...
    do {
        ...
        while (n == p || nodes[n].parent == p ||
               nodes[p].parent == n)  // n != p & n.parent != p
            p = rand() % modules_N;
...
void B_Tree::place_module(int mod, int abut, bool is_left) {
    ...
    if (is_left) {  // left
        int abut_width =
            (nodes[abut].rotate ? modules[abut].height : modules[abut].width);
    ...
    if (!node.isleaf()) {
        ...
        if (left) {
            ...
            if (node.right != NIL) {
                ...
                nodes[child].right =
                    node.right;  // abut with node's another child
            }
        } else {
            child = node.right;
            if (node.left != NIL) {
...
void B_Tree::insert_node2(Node& parent,
                          Node& node,
                          DIR edge,
                          DIR push,
                          bool fold) {

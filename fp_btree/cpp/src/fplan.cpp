...
double FPlan::getCost() {
    if (cost_alpha == 1)
        ...
        return cost_alpha * (Area / norm_area) +
               (1 - cost_alpha) * (WireLength / norm_wire);
}
...
void FPlan::create_network() {
    ...
    for (int j = 0; j < int(root_module.pins.size()); j++) {
...
void FPlan::show_modules() {
    for (int i = 0; i < int(modules.size()); i++) {
        ...
        //      cout << modules[i].pins[j].x << " " << modules[i].pins[j].y <<
        //      endl;
        //    }
    }
}
...
double seconds() {
    ...
    return (double)(1.0 * time.ru_utime.tv_sec +
                    0.000001 * time.ru_utime.tv_usec);
}
